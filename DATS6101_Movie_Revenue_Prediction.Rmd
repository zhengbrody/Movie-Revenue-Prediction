...
# Prepare data
X <- model.matrix( ~ ., data = train_data[predictors])
...
param_grid <- expand.grid(lambda = lambdas)

# Adjust parameters using cross validation
ridge_cv_model <- cv.glmnet(X, y, alpha = 0, lambda = param_grid$lambda)
lasso_cv_model <- cv.glmnet(X, y, alpha = 1, lambda = param_grid$lambda)

# Get the best lambda value
best_lambda_ridge <- ridge_cv_model$lambda.min
best_lambda_lasso <- ridge_cv_model$lambda.min
...
```{r ridge and lasso regression performance after tuning}
# Ridge and Lasso Regression
ridge_model_tuned <- glmnet(as.matrix(train_data[predictors]), train_data$Revenue, alpha=0, lambda=best_lambda_ridge)
lasso_model_tuned <- glmnet(as.matrix(train_data[predictors]), train_data$Revenue, alpha=1, lambda=best_lambda_lasso)

# Predictions
ridgetuned_train_predictions <- predict(ridge_model_tuned, newx = as.matrix(train_data[predictors]))
lassotuned_train_predictions <- predict(lasso_model_tuned, newx = as.matrix(train_data[predictors]))
ridgetuned_test_predictions <- predict(ridge_model_tuned, newx = as.matrix(test_data[predictors]))
lassotuned_test_predictions <- predict(lasso_model_tuned, newx = as.matrix(test_data[predictors]))
...
cat("Linear Regression Train Data RMSE:", lm_train_rmse, "\n")
cat("Linear Regression Test Data RMSE:", lm_test_rmse, "\n")
cat("Ridge Regression Train RMSE (tuned):", ridge_train_rmse, "\n")
cat("Ridge Regression Test RMSE (tuned):", ridge_test_rmse, "\n")
cat("Lasso Regression Train RMSE (tuned):", lasso_train_rmse, "\n")
cat("Lasso Regression Test RMSE (tuned):", lasso_test_rmse, "\n")
